...
import { Component, createResource, createSignal, JSXElement, Show, Suspense } from "solid-js"
import { getPercent, rpc } from "../utils"
import { GetMoveResponse } from "../../../shared/types"
import { OneMoveForm, BadgeTarget } from "./OneMoveForm"

const Card: Component<{ children: JSXElement; badge?: boolean; class?: string; onBadgeClick?: () => void }> = (props) => (
...
const AvatarRow: Component<{ svg: string; name: string; label: JSXElement }> = (props) => (
      ...
      <Show when={moveData()} keyed>
        {(data) => (
          <Show when={badgeTarget() === null} fallback={
            <OneMoveForm data={data} badgeTarget={badgeTarget()!} onCancel={() => setBadgeTarget(null)} />
          }>
            <OneMoveContent data={data} onBadgeClick={setBadgeTarget} />
          </Show>
        )}
      </Show>
    </Suspense>
...
const OneMoveContent: Component<{ data: GetMoveResponse; onBadgeClick: (target: BadgeTarget) => void }> = (props) => {
  const { move, claimStatement, statement, argument, avatar, nav, targetStatement, targetArgument, targetArgumentClaim } = props.data
